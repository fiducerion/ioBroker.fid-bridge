...
module.exports = function ({ adapter, broadcast, registerHostMessageHandler, getCfg }) {
  ...
  function scheduleNext() {
    ...
    scheduleTimer = setTimeout(() => {
      ...
      // Egal ob OK oder Fehler: naechsten Run planen
      scheduleNext();
    }, ms);
  }
  ...
  async function runBackup(trigger) {
    ...
    try {
      const cfg = getCfgBackup();
      const host = cfg.host || detectHost();
  ...
  router.get('/config', (req, res) => {
    res.json({ ok: true, config: getCfgBackup() });
  });

  router.post('/config', async (req, res) => {
    try {
      const saved = await saveCfgBackup(req.body || {});
      // adapter-restart nicht noetig: scheduleNext greift bei naechstem Tick
      // aber wir reschedule direkt
